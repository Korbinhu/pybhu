...
class HistogramDialog(QDialog):
    def __init__(self, state, on_limits_changed=None, parent=None):
        ...
        self.mode_selector.addItem("Global", userData="global")
        self.mode_selector.addItem("Per Layer", userData="per_layer")
        self._set_mode_selector_from_state()
        self.mode_selector.currentIndexChanged.connect(self._on_mode_index_changed)
    ...
    def _set_mode_selector_from_state(self):
        for i in range(self.mode_selector.count()):
            if self.mode_selector.itemData(i) == self.state.color_limit_mode:
                self.mode_selector.setCurrentIndex(i)
                break

    def _on_mode_index_changed(self, index):
