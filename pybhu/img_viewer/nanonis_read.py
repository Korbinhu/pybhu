...
from .nanonis_constants import nanonis_format_dict, nanonis_end_tags
...
def _parse_3ds_header(header_raw, header_override):
    ...
    try:
        ...
        if isinstance(header_dict['channels'], str):
            header_dict['channels'] = [header_dict['channels']]
        ...
        raise KeyError('[{key}] is missing from header.'.format(key=e.args[0]) + msg) from e
    except ValueError as e:
        msg = ' You can edit your header file or provide an override value in header_override'
        raise ValueError('Unexpected value found in header.' + msg) from e
