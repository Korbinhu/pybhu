...
class FitBundle:
    ...
    def __post_init__(self) -> None:
        ...
        for item in self.map_datasets:
            if isinstance(item, MapDataset):
                ...
                raise TypeError("map_datasets entries must be MapDataset or mapping-like objects")

            if dataset.data.shape[:2] != self.raw_cube.shape[:2]:
                raise ValueError(
                    f"map dataset {dataset.name!r} must match spatial shape {self.raw_cube.shape[:2]}"
                )
    ...
    def all_map_datasets(self) -> tuple[MapDataset, ...]:
        return (
            MapDataset("Raw Data", self.raw_cube, kind="raw_map"),
            MapDataset("Fitted Data", self.fit_cube, kind="fit_map"),
            *self.map_datasets,
        )
